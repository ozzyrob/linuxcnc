...
https://forum.linuxcnc.org/24-hal-components/38733-vfdmod-an-easy-vfd-control-over-modbus-rtu?start=0#162435
...
and the "modcompile" script will compile and install the module. For
reasons related to the fact that in some realtime implementations
LinuxCNC realtime modules are kernel modules where file access to
modules is not available mesa_modbus compiles-in the modbus register
structure and HAL pins into an immutable, loadable, module. If you
need to change the register assigments or add/remove pins/registers
...
same name but suffixed with -scale ond offset. The value from the
modbus register will be interpreted as a sighed-16bit number and then
it will be mutliplied by the scale value and then the offset will be
added (ie the offset is in local units, and the scale converts units)
...
pin type the registers will be promoted to float (whch can represent
integers up to 53 bits in length) and presented on a HAL pin with the
suffix "extnd". For example mymodule.0.counts-0-extnd.
...
modname.txdelay (default 20 bit lengths. generally be larger than Rx Delay)
...
to 16 if necessary. The Modbus protocol forces a hard max limit of 251,
but that would imply setting thousand of bits of hundreds of registers
