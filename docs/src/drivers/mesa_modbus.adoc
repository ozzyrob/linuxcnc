...
then be mutliplied by the scale value and then the offset will be
...
pin type the registers will be promoted to float (which can represent
integers up to 53 bits in length) and this wrap-adjusted value will
be presented on a HAL pin with the suffix "extnd".
For example mymodule.0.counts-0-extnd.
...
*modname.txdelay* (default 20 bit lengths. Generally shoiuld be larger than Rx Delay)
